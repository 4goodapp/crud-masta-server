	...
	<body>
		...
		<div id="show-city" class="content scaffold-show" role="main">
			...
			<ol class="property-list city">
				...
				<g:if test="${cityInstance?.province}">
				<li class="fieldcontain">
					<span id="province-label" class="property-label"><g:message code="city.province.label" default="Province" /></span>
					
						<span class="property-value" aria-labelledby="province-label"><g:link controller="province" action="show" id="${cityInstance?.province?.id}">${cityInstance?.province?.encodeAsHTML()}</g:link></span>
